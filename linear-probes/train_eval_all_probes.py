...
for dataset_name in datasets:
    ...
    probe = LinearProbe(input_dim=dataset.activation_size, device=device, learning_rate=config["probe_training_learning_rate"])
...
print(f"Probes trained and saved for all datasets. Starting evaluation...")

################### EVALUATING PROBES

for probe_dataset_name in datasets:
    print(f"Loading probe for {probe_dataset_name}...")
    ...
    for eval_dataset_name in [d for d in datasets if d != "AmongUsDataset"]: # simpler to eval on all other datasets
        print(f"Evaluating probe on {eval_dataset_name}...")

        eval_dataset = eval(f"{dataset_name}")(config, model=model, tokenizer=tokenizer, device=device, test_split=0.2, expt_name=amongus_expt_name)

        test_acts_chunk = dataset.get_test_acts(0)

        av_probe_outputs, _ = evaluate_probe_on_activation_dataset(
